...
namespace MulticastFunc
{
    ...
    public sealed class MulticastFunc<TArg1, TArg2, TResult>
    {
            ...
            [AllowNull] MulticastFunc<TArg1, TArg2, TResult> m)
        {
            ...
            if (m.Count == 1 && m.delegates[0] is Func<TArg1, TArg2, TResult> f)
                return f;

            return (Func<TArg1, TArg2, TResult>)Delegate.Combine(m.delegates)!;
        }
        #endregion
        ...
        /// <inheritdoc cref="Invoke(TArg1, TArg2)"/>
        ...
        {
            var results = new TResult[Count];
            FillBufferAndAggregateExceptions(arg1, arg2, results);
            return results;
        }

        /// <inheritdoc cref="InvokeAll(TArg1, TArg2)"/>
        public ReadOnlySpan<TResult> InvokeAll(TArg1 arg1, TArg2 arg2, Span<TResult> spanBuffer)
        {
            FillBufferAndAggregateExceptions(arg1, arg2, spanBuffer);
        ...
        private MulticastFunc<TArg1, TArg2, TResult>? Remove(Delegate[] functions)
        {
            ...
            if (ReferenceEquals(results, delegates))
                return this;
            // Some were removed
            return new MulticastFunc<TArg1, TArg2, TResult>(results);
        }
        ...
        private void FillBufferAndAggregateExceptions(TArg1 arg1, TArg2 arg2, Span<TResult> buffer)
        {
            var length = delegates.Length;
            if (buffer.Length < length)
                throw new ArgumentException("Buffer is too small", nameof(buffer));
